  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm divide-y divide-gray-100 dark:divide-gray-700">
      ...
      <div className="flex items-center justify-between px-5 py-4">
        <button onClick={onBack} className="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors flex items-center gap-1">
          ← Week
        </button>
        <div className="text-center">
          <p className="text-sm font-semibold text-gray-900 dark:text-white">Workout {session.workoutType}</p>
          <p className="text-xs text-gray-400 dark:text-gray-500">{format(parseISO(session.date), 'EEEE, MMM d')}</p>
        </div>
        <button
          ...
          <div key={exLog.exerciseId} className="px-5 py-4 space-y-3">
            <div className="flex items-start justify-between">
              ...
              {exLog.progressionFlag && (
                <span className="text-xs bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 font-medium px-2 py-1 rounded-full shrink-0 ml-2">
                  📈 Bump weight
                </span>
              )}
            </div>
...
function SetRow({
  ...
  return (
    <div className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${
      set.completed ? 'bg-emerald-50 dark:bg-emerald-900/20' : 'bg-gray-50 dark:bg-gray-700/50'
    }`}>
      <span className="text-xs text-gray-400 dark:text-gray-500 w-10 shrink-0">Set {set.setNumber}</span>
      ...
      {!isBodyweight && (
        <label className="flex items-center gap-1 shrink-0">
          <input
            ...
            className="w-16 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-md px-2 py-1 text-xs text-center focus:outline-none focus:ring-1 focus:ring-indigo-400 disabled:bg-transparent disabled:border-transparent"
          />
          <span className="text-xs text-gray-400 dark:text-gray-500">lbs</span>
        </label>
      )}
      ...
      <label className="flex items-center gap-1 shrink-0">
        <input
          ...
          className="w-16 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-md px-2 py-1 text-xs text-center focus:outline-none focus:ring-1 focus:ring-indigo-400 disabled:bg-transparent disabled:border-transparent"
        />
        <span className="text-xs text-gray-400 dark:text-gray-500">{isTimed ? 'sec' : 'reps'}</span>
      </label>
      ...
      <button
        ...
        className={`ml-auto w-7 h-7 rounded-full border-2 flex items-center justify-center text-xs font-bold transition-colors ${
          set.completed
            ...
            : 'border-gray-300 dark:border-gray-600 text-transparent hover:border-emerald-400 disabled:opacity-40'
        }`}
      >
        ✓
      </button>
