...
export function CustomMealForm({ onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-gray-800 w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl shadow-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700">
          <h3 className="font-semibold text-gray-900 dark:text-white">Create Custom Meal</h3>
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
            ...
            {([['kcal', 'Calories', 'kcal'], ['protein', 'Protein', 'g'], ['carbs', 'Carbs', 'g'], ['fat', 'Fat', 'g']] as const).map(([field, label, unit]) => (
              <div key={field}>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs text-gray-500 dark:text-gray-400 shrink-0">{unit}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose}
              className="flex-1 border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400 rounded-lg py-2 text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
              Cancel
            </button>
            <button type="submit"
