    ...
    <div className="p-6 max-w-xl mx-auto space-y-5">
      ...
      <div className="bg-white rounded-xl shadow-sm p-5">
        ...
        <div className="space-y-2 text-sm text-gray-600">
          <div className="flex justify-between"><span>Height</span><span className="font-medium text-gray-900">{Math.floor(profile.heightInches / 12)}′{profile.heightInches % 12}″</span></div>
          <div className="flex justify-between"><span>Age</span><span className="font-medium text-gray-900">{profile.age}</span></div>
          <div className="flex justify-between"><span>Dairy-free</span><span className="font-medium text-gray-900">{profile.dairyFree ? 'Yes' : 'No'}</span></div>
          <div className="flex justify-between"><span>Egg-free</span><span className="font-medium text-gray-900">{profile.eggFree ? 'Yes' : 'No'}</span></div>
          <div className="flex justify-between"><span>Avocado-free</span><span className="font-medium text-gray-900">{profile.avocadoFree ? 'Yes' : 'No'}</span></div>
        </div>
        <button onClick={() => router.push('/setup')}
          className="mt-4 w-full text-center text-sm text-indigo-600 hover:text-indigo-700 font-medium transition-colors">
          Re-run setup wizard →
        </button>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-5 border border-red-100">
        ...
        {!showReset ? (
          ...
          <div className="space-y-3">
            <p className="text-sm text-gray-600">This will delete all meals, workouts, and weight entries. Are you sure?</p>
